...
# total_rental_cost     rental_duration * rental_rate
select title, description, release_year, rental_rate*rental_duration as total_rental_cost from film where length(description) between 100 and 120;
...
select payment_id, payment_date, amount from payment where amount > 5 limit 50 offset 51;
...
(select payment_date, amount from payment limit 20) order by amount desc;

# 8d. Select the title, description, special features, length, and rental duration columns from the film table for the first 10 films with behind the scenes footage under 2 hours in length 
# and a rental duration between 5 and 7 days, ordered by length in descending order.
(select title, description, special_features, length, rental_duration from film where special_features='Behind the Scenes' and length<120 and rental_duration between 5 and 7 limit 10) order by length desc;
...
# 9f. Select the first_name, last_name, address, address2, city name, district, and postal code columns from the staff table, performing 2 left joins with the address table then the city table
